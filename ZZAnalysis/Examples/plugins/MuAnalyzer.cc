...
void MuAnalyzer::beginJob(){
  ...
  TFileDirectory GlobalPresel = spectra.mkdir("GlobalPresel muons");
  TFileDirectory GlobalNjet = spectra.mkdir("GlobalNjet muons");
  TFileDirectory GlobalZllmass = spectra.mkdir("GlobalZllmass muons");
  TFileDirectory GlobalNmu = spectra.mkdir("GlobalNmu muons");
  ...
  //Global muons (presel)
  h1PtRecoGlobalPreselMuon_ = GlobalPresel.make<TH1F>("pTGlobalPreselMuons","p_{T}^{rec}",250,0,120);
  h1EtaRecoGlobalPreselMuon_ = GlobalPresel.make<TH1F>("etaGlobalPreselMuons","p_{T}^{rec}",20,0,4);
  h1PhiRecoGlobalPreselMuon_ = GlobalPresel.make<TH1F>("phiGlobalPreselMuons","p_{T}^{rec}",50,0,3.15);
  h1DxyRecoGlobalPreselMuon_ = GlobalPresel.make<TH1F>("dxyGlobalPreselMuons","d_{xy}^{rec}",400,-200,200);
  h1DzRecoGlobalPreselMuon_ = GlobalPresel.make<TH1F>("dzGlobalPreselMuons","d_{z}^{rec}",400,-200,200);
  h2EtaPtRecoGlobalPreselMuon_ = GlobalPresel.make<TH2F>("EtaPtGlobalPreselMuon","Eta vs Pt",20,0,4,250,0,120);
  //Global muons (njet > 1)
  h1PtRecoGlobalNjetMuon_ = GlobalNjet.make<TH1F>("pTGlobalNjetMuons","p_{T}^{rec}",250,0,120);
  h1EtaRecoGlobalNjetMuon_ = GlobalNjet.make<TH1F>("etaGlobalNjetMuons","p_{T}^{rec}",20,0,4);
  h1PhiRecoGlobalNjetMuon_ = GlobalNjet.make<TH1F>("phiGlobalNjetMuons","p_{T}^{rec}",50,0,3.15);
  h1DxyRecoGlobalNjetMuon_ = GlobalNjet.make<TH1F>("dxyGlobalNjetMuons","d_{xy}^{rec}",400,-200,200);
  h1DzRecoGlobalNjetMuon_ = GlobalNjet.make<TH1F>("dzGlobalNjetMuons","d_{z}^{rec}",400,-200,200);
  h2EtaPtRecoGlobalNjetMuon_ = GlobalNjet.make<TH2F>("EtaPtGlobalNjetMuon","Eta vs Pt",20,0,4,250,0,120);
  //Global muons (zll mass > 20)
  h1PtRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH1F>("pTGlobalZllmassMuons","p_{T}^{rec}",250,0,120);
  h1EtaRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH1F>("etaGlobalZllmassMuons","p_{T}^{rec}",20,0,4);
  h1PhiRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH1F>("phiGlobalZllmassMuons","p_{T}^{rec}",50,0,3.15);
  h1DxyRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH1F>("dxyGlobalZllmassMuons","d_{xy}^{rec}",400,-200,200);
  h1DzRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH1F>("dzGlobalZllmassMuons","d_{z}^{rec}",400,-200,200);
  h2EtaPtRecoGlobalZllmassMuon_ = GlobalZllmass.make<TH2F>("EtaPtGlobalZllmassMuon","Eta vs Pt",20,0,4,250,0,120);
  //Global muons (nmu > 2)
  h1PtRecoGlobalNmuMuon_ = GlobalNmu.make<TH1F>("pTGlobalNmuMuons","p_{T}^{rec}",250,0,120);
  h1EtaRecoGlobalNmuMuon_ = GlobalNmu.make<TH1F>("etaGlobalNmuMuons","p_{T}^{rec}",20,0,4);
  h1PhiRecoGlobalNmuMuon_ = GlobalNmu.make<TH1F>("phiGlobalNmuMuons","p_{T}^{rec}",50,0,3.15);
  h1DxyRecoGlobalNmuMuon_ = GlobalNmu.make<TH1F>("dxyGlobalNmuMuons","d_{xy}^{rec}",400,-200,200);
  h1DzRecoGlobalNmuMuon_ = GlobalNmu.make<TH1F>("dzGlobalNmuMuons","d_{z}^{rec}",400,-200,200);
  h2EtaPtRecoGlobalNmuMuon_ = GlobalNmu.make<TH2F>("EtaPtGlobalNmuMuon","Eta vs Pt",20,0,4,250,0,120);
  //Only Tracker muons
  ...
  for (muon1 = muonsHandle->begin();  muon1 != muonsHandle->end(); ++muon1){
    ...
    if( muon1->isGlobalMuon() ){
      ...
      if ( muon1->pt() > 10. && 
	   ...
	   muon1->globalTrack()->hitPattern().numberOfValidMuonHits() > 0 && 
	   muon1->dB() < 0.2 && (muon1->trackIso()+muon1->caloIso()) < 0.15*muon1->pt() && muon1->numberOfMatches() > 1 && abs(muon1->eta()) < 2.1){
	h1PtRecoGlobalPreselMuon_->Fill(ptMu);
	h1EtaRecoGlobalPreselMuon_->Fill(etaMu);
	h1PhiRecoGlobalPreselMuon_->Fill(phiMu);
	h1DxyRecoGlobalPreselMuon_->Fill(muon1->globalTrack()->d0());
	h1DzRecoGlobalPreselMuon_->Fill(muon1->globalTrack()->dz());
	h2EtaPtRecoGlobalPreselMuon_->Fill(etaMu,ptMu);
      }

      //GlobalNjet
      if( jetsPFHandle->size() > 1 ) {
	h1PtRecoGlobalNjetMuon_->Fill(ptMu);
	h1EtaRecoGlobalNjetMuon_->Fill(etaMu);
	h1PhiRecoGlobalNjetMuon_->Fill(phiMu);
	h1DxyRecoGlobalNjetMuon_->Fill(muon1->globalTrack()->d0());
	h1DzRecoGlobalNjetMuon_->Fill(muon1->globalTrack()->dz());
	h2EtaPtRecoGlobalNjetMuon_->Fill(etaMu,ptMu);
      }

      //GlobalZllmass
      if( zll.size() == 1 && zll[0].mass() > 20){
	h1PtRecoGlobalZllmassMuon_->Fill(ptMu);
	h1EtaRecoGlobalZllmassMuon_->Fill(etaMu);
	h1PhiRecoGlobalZllmassMuon_->Fill(phiMu);
	h1DxyRecoGlobalZllmassMuon_->Fill(muon1->globalTrack()->d0());
	h1DzRecoGlobalZllmassMuon_->Fill(muon1->globalTrack()->dz());
	h2EtaPtRecoGlobalZllmassMuon_->Fill(etaMu,ptMu);
      }

      //GlobalNmu
      if( muonsHandle->size() > 2 ){
	h1PtRecoGlobalNmuMuon_->Fill(ptMu);
	h1EtaRecoGlobalNmuMuon_->Fill(etaMu);
	h1PhiRecoGlobalNmuMuon_->Fill(phiMu);
	h1DxyRecoGlobalNmuMuon_->Fill(muon1->globalTrack()->d0());
	h1DzRecoGlobalNmuMuon_->Fill(muon1->globalTrack()->dz());
	h2EtaPtRecoGlobalNmuMuon_->Fill(etaMu,ptMu);
      }
