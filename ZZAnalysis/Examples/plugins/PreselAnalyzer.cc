...
void PreselAnalyzer::beginJob(){
  ...
  TFileDirectory subspectra4 = spectra.mkdir("Non presel muons in many muons events");
  ...
  h1DiMuMassManyPreselMuEvts_ = subspectra4.make<TH1F>("DiMuMassManyPreselMuEvts","Dimuon mass spectrum of muons pair (opposite charge) in events with more than 2 presel muons",100,0,120);
  ...
  //NPreselMuPerEvent histogram
  h1NPreselMuPerEvent_ = counters.make<TH1I>("NPreselMuPerEvent","Number of PreselMu per event",20,-0.5,19.5);
}
...
void PreselAnalyzer::analyze(const Event & iEvent, const EventSetup& eventSetup)
{
  ...
  for (muon = muonsHandle->begin();  muon != muonsHandle->end(); ++muon){
	...
	for (muon2 = muonsHandle->begin();  muon2 != muonsHandle->end(); ++muon2){
	...
	if ( muon->outerTrack()->hitPattern().numberOfValidMuonHits() > 0 &&
      ...
      if ( muon->pt() <= 5. ) h1PreselFail_->Fill(0);
  ...
  //Study the reason why we have just one Z candidate per event
  int PreselCounter = 0;
  if ( zllHandle->size() == 1 ){
    for (muon = muonsHandle->begin();  muon != muonsHandle->end(); ++muon){
      //Let muon pass the preselection
      if ( muon->pt() > 5. && 
	   muon->isGlobalMuon()==true && muon->isTrackerMuon()==true && 
	   muon->normChi2() < 15 && muon->innerTrack()->hitPattern().numberOfValidTrackerHits() > 10 && muon->innerTrack()->hitPattern().numberOfValidPixelHits() > 0  &&
	   muon->globalTrack()->hitPattern().numberOfValidMuonHits() > 0 && 
	   muon->dB() < 0.2 && (muon->trackIso()+muon->caloIso()) < 0.15*muon->pt() && muon->numberOfMatches() > 1 && abs(muon->eta()) < 2.1)PreselCounter++;
    }//end loop on muon collection
  }//end if Zll->size() > 2

      //Fill NPreselMuPerEvent
      h1NPreselMuPerEvent_->Fill(PreselCounter);
	...
	for (muon = muonsHandle->begin();  muon != muonsHandle->end(); ++muon){
	  ...
	  if ( muon->pt() > 5. && 
	       ...
	       muon->globalTrack()->hitPattern().numberOfValidMuonHits() > 0 && 
	       muon->dB() < 0.2 && (muon->trackIso()+muon->caloIso()) < 0.15*muon->pt() && muon->numberOfMatches() > 1 && abs(muon->eta()) < 2.1){
	    for (muon2 = muonsHandle->begin();  muon2 < muon; ++muon2){
	      //Let muon2 pass the preselection
	      if ( muon2->pt() > 5. && 
		...
		sum = p1 + p2;
		CandMass = sum.mass();
		if((muon->charge()*muon2->charge())<0 && CandMass > 8) h1DiMuMassManyPreselMuEvts_->Fill(CandMass);
	      }//end if preselection
