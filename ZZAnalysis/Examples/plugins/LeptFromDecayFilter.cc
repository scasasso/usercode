...
LeptFromDecayFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for( GenParticleCollection::const_iterator genp = genParticlesCollection->begin();genp != genParticlesCollection->end(); ++ genp ){
	...
	p = p->mother();
      ...
      //Increase counter muons from beauty hadrons
      if( 	   (abs(p->mother()->pdgId()) > 511 && abs(p->mother()->pdgId()) < 545) ||// b-mesons
	...
	h1PtFromB_->Fill(p->pt());
      ...
      //Increase counter muons from charm hadrons
	...
	if((abs(p->mother()->mother()->pdgId()) > 511 && abs(p->mother()->mother()->pdgId()) < 545) ||// b-mesons
	   (abs(p->mother()->mother()->pdgId()) > 551 && abs(p->mother()->mother()->pdgId()) < 557) ||// bbbar
	   (abs(p->mother()->mother()->pdgId()) > 5122 && abs(p->mother()->mother()->pdgId()) < 5554)){
	  nbcMu_++;
      ...
      if( abs(p->mother()->pdgId()) == 23 )h1PtFromZ_->Fill(p->pt());
	...
	p = p->mother();
      ...
      //Increase counter electrons from beauty hadrons
      if( 	   (abs(p->mother()->pdgId()) > 511 && abs(p->mother()->pdgId()) < 545) ||// b-mesons
		   (abs(p->mother()->pdgId()) > 551 && abs(p->mother()->pdgId()) < 557) ||// bbbar
		   (abs(p->mother()->pdgId()) > 5122 && abs(p->mother()->pdgId()) < 5554) // b-barions
		   ){
	...
	h1PtFromB_->Fill(p->pt());
      ...
      //Increase counter electrons from charm hadrons
      if( (abs(p->mother()->pdgId()) > 411 && abs(p->mother()->pdgId()) < 435) || // c-mesons
	  (abs(p->mother()->pdgId()) > 441 && abs(p->mother()->pdgId()) < 445)  ||// ccbar
	  (abs(p->mother()->pdgId()) > 4122 && abs(p->mother()->pdgId()) < 4444)// c-barions
	  ){
  ...
  if( tokens[1] == "m" ) {
    hasMu_ = true;
    //cout<<"I'm interested in muons"<<endl;
  }

  if( tokens[1] == "e" ) {
    hasEle_ = true;
    //cout<<"I'm interested in electrons"<<endl;
  }
  ...
  if ( tokens[0] == "b" ) {
    ...
    hasBC_ = true;
    //cout<<"grandmother: b"<<endl<<"mother: c"<<endl;
  }
  ...
  if ( hasMu_ && !(hasEle_)){//if interested in muons
    ...
    if ( hasB_ ){
      if ( nbMu_ != 0 ) decision_ = true;
      else decision_ = false;
    }
    else {};
    
    if ( hasC_ ){
      if ( ncMu_ != 0) decision_ = true;
      else decision_ = false;
    }
    else {};

    if ( hasBC_ ){
      if ( nbcMu_ != 0) decision_ = true;
      else decision_ = false;
    }
    else {};
  }
  ...
  if ( hasEle_ && !(hasMu_)){//if interested in electrons
    ...
    if ( hasB_ ){
      if ( nbEle_ != 0 ) decision_ = true;
      else decision_ = false;
    }
    else {};

    if ( hasC_ ){
      if ( ncEle_ != 0) decision_ = true;
      else decision_ = false;
    }
    else {};

    if ( hasBC_ ){
      if ( nbcEle_ != 0) decision_ = true;
      else decision_ = false;
    }
    else {};
  }
  ...
  if ( decision_ ){
    return true;
    nPassedEvts_++;
    //cout<<"yes it passed"<<endl;
  }
  else {
    return false;
    nNotPassedEvts_++;
    //cout<<"no it didn't pass"<<endl;
  }
}
